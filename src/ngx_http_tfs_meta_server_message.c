<<<<<<< HEAD:ngx_http_tfs_meta_server_message.c
...
ngx_http_tfs_select_meta_server(ngx_http_tfs_t *t)
{
    ...
    hash = ngx_http_tfs_murmur_hash((u_char *) &h, sizeof(ngx_http_tfs_meta_hh_t));
...
ngx_http_tfs_parse_ls_message(ngx_http_tfs_t *t)
{
    ...
    if (count == 0) {
        if (t->r_ctx.action.code == NGX_HTTP_TFS_ACTION_LS_FILE) {
            ngx_log_error(NGX_LOG_DEBUG, t->log, 0, "file(%V) not exist", &t->r_ctx.file_path_s);
            return NGX_HTTP_TFS_EXIT_TARGET_EXIST_ERROR;
        } else {
            return NGX_OK;
        }
    ...
    meta_info = &t->meta_info;
    if (meta_info->files == NULL) {
        meta_info->files = ngx_pcalloc(t->pool, sizeof(ngx_http_tfs_custom_file_t) * count);
        if (meta_info->files == NULL) {
            return NGX_ERROR;
        }
        meta_info->file_count = count;
        ...
        if (meta_info->file_index == meta_info->file_count) {
            new_meta_info = &meta_info->next;
            meta_info = ngx_pcalloc(t->pool, sizeof(ngx_http_tfs_custom_meta_info_t));
            if (meta_info == NULL) {
                return NGX_ERROR;
            }
            meta_info->files = ngx_pcalloc(t->pool, sizeof(ngx_http_tfs_custom_file_t) * count);
    ...
    for (i = meta_info->file_index; i < meta_info->file_count && p < (b->last - sizeof(uint32_t)); i++) {
        file[i].file_name.len = *((uint32_t *) p); /* include '\0'*/
        p += sizeof(uint32_t);
        if (p + file[i].file_name.len >= b->last) {
            p -= sizeof(uint32_t);
        ...
        if (file[i].file_name.len > 0) {
            file[i].file_name.data = ngx_pcalloc(t->pool, file[i].file_name.len - 1);
            ngx_memcpy(file[i].file_name.data, p, file[i].file_name.len - 1);
            p += file[i].file_name.len;
        ...
        ngx_memcpy(&file[i].file_info, p, sizeof(ngx_http_tfs_custom_file_info_t));
        p += sizeof(ngx_http_tfs_custom_file_info_t);
    }
    ...
    if (t->r_ctx.action.code == NGX_HTTP_TFS_ACTION_LS_FILE) {
        ...
        t->last_file_type = (((t->last_file_pid >> 63) & 0x01) == 0x01) ?
            NGX_HTTP_TFS_CUSTOM_FT_FILE : NGX_HTTP_TFS_CUSTOM_FT_DIR;
        ngx_log_error(NGX_LOG_DEBUG, t->log, 0, "ls last file path: %V", &t->last_file_path);
    }

    return NGX_OK;
}
>>>>>>> 88b88637f9b6aa208a109d18da5113750b4ba69e:src/ngx_http_tfs_meta_server_message.c
